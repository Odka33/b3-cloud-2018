  ...
  (1..$num_instances).each do |i|
    config.vm.define vm_name = "%s-%02d" % [$instance_name_prefix, i] do |config|
      ...
      config.vm.provider :virtualbox do |vb|
        ...
        vb.memory = vm_memory
        vb.cpus = vm_cpus
        vb.customize ['storagectl', :id, '--name', 'SATACtrl', '--add', 'sata', '--portcount', 1]
        file_to_disk = "disk#{i}.vdi"
        unless File.exist?(file_to_disk)
          vb.customize ['createhd', '--filename', file_to_disk, '--size', 10 * 1024]
        end
        vb.customize ['storageattach', :id, '--storagectl', 'SATACtrl', '--port', 0, '--device', 0, '--type', 'hdd', '--medium', file_to_disk] 
